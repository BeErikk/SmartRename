...
CMockSmartRenameRegExEvents::AddRef()
{
	return InterlockedIncrement(&m_refCount);
}

IFACEMETHODIMP_(ULONG)
CMockSmartRenameRegExEvents::Release()
{
...
IFACEMETHODIMP CMockSmartRenameRegExEvents::OnSearchTermChanged(_In_ PCWSTR searchTerm)
{
	...
	{
		::SHStrDupW(searchTerm, &m_searchTerm);
	}
	return S_OK;
}
...
IFACEMETHODIMP CMockSmartRenameRegExEvents::OnReplaceTermChanged(_In_ PCWSTR replaceTerm)
{
	...
	{
		::SHStrDupW(replaceTerm, &m_replaceTerm);
	}
	return S_OK;
}

IFACEMETHODIMP CMockSmartRenameRegExEvents::OnFlagsChanged(_In_ uint32_t flags)
{
	m_flags = flags;
	return S_OK;
}
