...
IFACEMETHODIMP CSmartRenameRegEx::Advise(_In_ ISmartRenameRegExEvents* regExEvents, _Out_ uint32_t* cookie)
{
...
IFACEMETHODIMP CSmartRenameRegEx::UnAdvise(_In_ uint32_t cookie)
{
	HRESULT hr = E_FAIL;
	CSRWExclusiveAutoLock lock(&m_lockEvents);
...
IFACEMETHODIMP CSmartRenameRegEx::get_searchTerm(_Outptr_ PWSTR* searchTerm)
{
	*searchTerm = nullptr;
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = ::SHStrDupW(m_searchTerm, searchTerm);
	}
	return hr;
}
...
IFACEMETHODIMP CSmartRenameRegEx::put_searchTerm(_In_ PCWSTR searchTerm)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (m_searchTerm == nullptr || lstrcmp(searchTerm, m_searchTerm) != 0)
		{
			...
			hr = ::SHStrDupW(searchTerm, &m_searchTerm);
		}
	}
...
IFACEMETHODIMP CSmartRenameRegEx::get_replaceTerm(_Outptr_ PWSTR* replaceTerm)
{
	*replaceTerm = nullptr;
	...
	{
		CSRWSharedAutoLock lock(&m_lock);
		hr = ::SHStrDupW(m_replaceTerm, replaceTerm);
	}
	return hr;
}
...
IFACEMETHODIMP CSmartRenameRegEx::put_replaceTerm(_In_ PCWSTR replaceTerm)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (m_replaceTerm == nullptr || lstrcmp(replaceTerm, m_replaceTerm) != 0)
		{
			...
			hr = ::SHStrDupW(replaceTerm, &m_replaceTerm);
		}
	}
...
IFACEMETHODIMP CSmartRenameRegEx::get_flags(_Out_ uint32_t* flags)
{
	*flags = m_flags;
	return S_OK;
}

IFACEMETHODIMP CSmartRenameRegEx::put_flags(_In_ uint32_t flags)
{
	if (m_flags != flags)
	{
		m_flags = flags;
	...
	: m_refCount(1)
{
	...
	::SHStrDupW(L"", &m_searchTerm);
	::SHStrDupW(L"", &m_replaceTerm);
}
